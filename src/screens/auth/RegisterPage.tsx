...
export default function SignupScreen({ navigation }: any) {
  ...
  // gains focus, so it never leaks into the Signup UI.
  useFocusEffect(
    useCallback(() => {
      ...
      >
        <Box flex={1} bg="#F8F9FB" onLayout={onLayout} safeArea>
          {containerDimensions.baseSize > 0 && (
            // Center aligns everything perfectly in the middle of the screen
            <Center flex={1} width={"100%"} px={isTablet ? "8%" : "6%"} py={10}>
              {/* Main Form Stack - Notice there is no flex={X} here, so it wraps its content tightly */}
              <VStack
                w={"100%"}
                maxW={`${formMaxWidth}px`}
                space={sz(0.04, 4, 6)} // Consistent spacing between major sections
              >
                ...
                {/* 2. Inputs Group - Tight spacing (space=3) to keep fields compact */}
                ...
                <VStack space={3}>
                  <Checkbox
                    ...
                    colorScheme="indigo"
                  >
                    <Text fontSize={sz(0.032, 12, 14)} color="#374151" ml={1}>
