...
export default function ProjectDetailScreen({ route }: any) {
  ...
  // ✅ Master flag for completed status
  ...
  // If loading is done, but there is strictly no project found
  if (!loading && !project) {
    return (
      <Center flex={1}>
        <Text color="coolGray.400">Project not found</Text>
      </Center>
    );
  }
      ...
      >
        <ScrollView
          ...
          contentContainerStyle={{
            width: "100%",
            marginTop: adjustSizeToResolveZoomInIssue(
              containerDimensions.height * 0.02,
            ),
        ...
        >
          <Box justifyContent={"center"} width="98%">
            {/* ══════════════════════════════════════════
                ...
                >
                  ...
                  >
                    <NBAvatar
                      ...
                      h={fs.icon}
                      bg="indigo.500"
                      source={{ uri: project.admin?.profileImgUrl }}
                    >
            ...
            >
              ...
              >
                ...
                <VStack
                  ...
                  flex={1}
                >
                  {/* ✅ FIX: Hide edit pencil if completed */}
                  {isAdmin && (
              ...
              {[
                {
                  ...
                  show: isAdmin, // ✅ FIX: Hide if completed
                },
                {
                  ...
                  show: isAdmin || isEditor, // ✅ FIX: Hide if completed
                },
                {
                  ...
                  show: true, // Messages component handles disabling its own input
                },
                {
                    ...
                    {({ isPressed }) => (
                      ...
                      >
                        <Icon
                          ...
                          color={
                            isCompleted ? "coolGray.200" : action.iconColor
                          }
                        />
                        <Text
                          fontSize={fs.meta}
            ...
            {taskError ? (
              ...
              >
                {tasks
                  ?.slice(0, DEFAULT_RECENT_TASK_LIMIT)
                  .map((task, index) => {
                    const tPriority =
            ...
            {/* ══════════════════════════════════════════
                 SECTION 8 — Team Members (SEPARATED)
            ══════════════════════════════════════════ */}
            <ProjectTeamMembers
              projectId={project.projectId}
              userId={user!.userId}
